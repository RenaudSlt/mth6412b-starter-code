...
""" Algorithme de Prim : supporté par une structure de donnée MarkedNode{T} dans markednode.jl 
    ...
    -Input : 
      1) graphe connexe ayant des sommets et des arcs 
      2) un noeud source
    -Output : 
      1) ensemble d'arêtes formant le recouvrement minimal 
      2) poids de l'arbre recouvrant minimal
      3) vecteur de MarkedNodes formant l'arbre de recouvrement minimal
"""
function prim_algorithm(graph::Graph{T}, source::Node{T}=get_nodes(graph)[1]) where T
    ...
    return edges_graph_min, weight_min, visited_nodes
end