...
function get_root!(node::NodeTree{T}; compression::Bool=true) where T 
    ...
    if get_parent(node) === nothing  
        ...
        if compression
            set_parent!(node, get_root!(get_parent(node); compression=compression))  
            return get_parent(node)
        # Sans compression
        else 
            get_root!(get_parent(node); compression=compression)         
        end
...
function same_tree(node1::NodeTree{T}, node2::NodeTree{T}; compression::Bool=true) where T 
    root_tree1 = get_root!(node1; compression=compression)
    root_tree2 = get_root!(node2; compression=compression)
